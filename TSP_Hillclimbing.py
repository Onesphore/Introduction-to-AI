...
def Main():
    ...
    fout = open( os.path.dirname(os.path.abspath(__file__)) +"/result.txt","w")		# result file open
    ...
    total_distance = hillClimbing(nOfCity,distances,route,total_distance)       # solving the problem by hill climbing
    ...
    print ("total_distance = ", total_distance)                                 # printing the total_distance found on the monitor
	
	
def hillClimbing(nOfCity,distances,route,total_distance):
    ...
    while(1):
        ...
        route_it = itertools.combinations(route[1:10],2)    #I used itertools.combinations to make all possible combinations
        route_combinations = []                             #I imported itertools
        ...
        cities_to_swap = (1,1)
        better_total_distance = total_distance   #we try to find a better total distance, when it is found we
        for k in route_combinations:             # immediately break the for loop
            route_copy = route[:]
            ...
            if(new_total_distance<better_total_distance):
                better_total_distance = new_total_distance
                cities_to_swap = k
                break


        if(better_total_distance<total_distance):  #the total_distance is updated and so is the route
            ...
            route[x1] = route[x2]
            route[x2] = holder
            total_distance = better_total_distance
